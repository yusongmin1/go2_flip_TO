"""
Export Go2 trajectories in the same JSON ``Frames`` format as ``datasets/ai.py``
(``save_as_txt_with_metadata``): 25 Hz by default, **Pinocchio q / v joint order** (URDF order).

Each frame row (49 floats) matches ``ai.py`` ``out43`` layout:
  [ q (nq), foot_xyz in base x4, lin_b (3), ang_b (3), joint_vel (12) ]
  = 19 + 12 + 6 + 12
"""
...
def compose_amp49_row(model: pin.Model, data: pin.Data, q: np.ndarray, v: np.ndarray) -> np.ndarray:
    ...
    (ω, v from generalized v[:6]), joint rates ``v[6:]`` in **same order as** ``q[7:]``.
    """
    q = np.asarray(q, dtype=np.float64).ravel()
    ...
    # Floating-base part of v: spatial velocity in local (base) frame, (angular, linear)
    ang_b = np.asarray(v[0:3], dtype=np.float64).ravel()
    lin_b = np.asarray(v[3:6], dtype=np.float64).ravel()
        ...
        json.dumps(
            {
                ...
                "layout": {
                    ...
                    "columns_31_33": "base linear velocity in base frame (v[3:6])",
                    "columns_34_36": "base angular velocity in base frame (v[0:3])",
