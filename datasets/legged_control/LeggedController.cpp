...
namespace legged {
  void writeStateRow(const vector_t& pos,
                   const vector_t& quat,
                   const vector_t& linVel,
                   const vector_t& angVel,
                   const vector_t& jPos,
                   const vector_t& jVel)
{
    std::ofstream fout;
    fout.open("/home/zju/result.txt", std::ios::app);          // 追加模式
    if (!fout.is_open()) {
        std::cerr << "无法创建/打开 result.txt\n";
        return;
    }

    /* 设置格式：空格分隔，不科学计数，保留 6 位小数 */
    ...
    /* 4. 线速度 3 */
    fout << linVel.transpose() << ' ';

    /* 5. 角速度 3 */
    fout << angVel.transpose() << ' ';
...
void LeggedController::updateStateEstimation(const ros::Time& time, const ros::Duration& period) {
  ...
  writeStateRow(stateEstimate_->rbdState_.segment<3>(3),quat.coeffs(),
  stateEstimate_->rbdState_.segment<3>(18 + 3),angularVel,
  jointPos,jointVel);
}
