...
def build_and_solve():
    ...
    # 起始站稳 0.4 s → 下蹲 0.5 s（只许蹲）→ 蓄力 0.1 s（最低点速度≈0）
    # → 腾空 0.55 s → 落地缓冲 0.6 s → 结束站稳 0.4 s
    ...
    # warm start（阶段 A 用）：翻滚角线性展开（沿用原始脚本）
    ...
    def _solve_chained(tag, max_rounds=3):
        ...
        for _round in range(max_rounds):
            ...
            print(
                f"[quad_sideflip_right] {tag} round {_round + 1}: {res['solve_time']:.1f} s "
    ...
    # ---- 阶段 B：以下蹲时序约束（站稳带 + 单调坡道 + 最低点竖直速度带）重解，
    # 暖启动 = 阶段 A 的解。两组约束直接从冷启动解会在 300 轮内不收敛 ----
    _lb_backup = list(opti.lb)
    _ub_backup = list(opti.ub)
    ...
    for _k in range(k0, k_load):
        ...
        # 只用 thigh 下界坡道保证"单调下蹲、不许比站立更直"；不加 calf 上界 ——
        # 阶段 A 的自然深蹲 calf 到 -3.4，calf 封顶会挡住深蹲导致不可解
    ...
    # 最低点速度≈0 不额外加带：下蹲最低点本身就是 vz 过零的转折点（阶段 A 自然
    # 满足），硬加速度带会打乱整个下蹲节拍（实测 300 轮不收敛）

    opti.x0 = np.asarray(opti.sol, dtype=float).copy()
    # 暖启动也按坡道剪裁（阶段 A 的解可能违反新边界）
    for _k in range(0, k_load):
        _node = opti.nodes[_k]
        for _jn, _side in (("FL_thigh_joint", "th"), ("FR_thigh_joint", "th"),
                           ("RL_thigh_joint", "th"), ("RR_thigh_joint", "th"),
                           ("FL_calf_joint", "ca"), ("FR_calf_joint", "ca"),
                           ("RL_calf_joint", "ca"), ("RR_calf_joint", "ca")):
            _iq = robot.model.joints[robot.model.getJointId(_jn)].idx_q
            _ti = _node.q_id.start + 6 + (_iq - 7)
